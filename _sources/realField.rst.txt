...
For any real :math:`a,~b` with :math:`a\neq 0,~b\neq 0`,
   
.. math::

   b^{-1}\cdot a^{-1}\cdot a\cdot b &= b^{-1}\cdot \left(a^{-1}\cdot a\right)\cdot b = b^{-1}\cdot b =1

   &\implies

   (a\cdot b)^{-1} &= b^{-1}\cdot a^{-1} = a^{-1}\cdot b^{-1}

This can be extended to more than two invertible real numbers.

.. math::

   (a\cdot b\cdot c\cdot d)^{-1} &= d^{-1}\cdot c^{-1}\cdot b^{-1}\cdot a^{-1} = a^{-1}\cdot b^{-1}\cdot c^{-1}\cdot d^{-1}
...
We do not get into the axiomatics of integers here. However, we will assume that integers are a subset of real numbers that get inductively defined as follows:
...
This helps us use results such as :math:`a+a=2\cdot a`.
