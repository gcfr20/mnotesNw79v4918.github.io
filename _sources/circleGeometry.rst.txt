...
.. image:: geom_figs/circle/inscribed_ang_thm.png
  :width: 600

A part of the circumference of the circle is called an "arc". In the above figure, BGC is an arc. The arc is measured by the angle it subtends at the center of the circle. So arc BGC has measure :math:`108^o` (the measure of angle BAC).
...
The measure of arc BDC is :math:`360^o-\angle BAC=252^o`. Angles BGC anf BHC are inscribed angles subtended by arc BDC (blue angles). Their measure is half of the measure of arc BDC i.e. :math:`126^o`.

Note that :math:`\angle BCF` is formed by the chord that connects the ends of the arc BGC and the tangent at point C. The measure of this angle is also half the arc measure. Similarly :math:`\angle BCI` is half the measure of arc BDC.

======================================

.. image:: geom_figs/circle/diameter.png
  :width: 400

A diameter can be thought of as the angle subtended by the semicircle at the center of the circle. This subtended angle is :math:`180^o`. Hence, any inscribed angle subtended by the semicircle of the diameter is equal to :math:`90^o`. Hence, :math:`\angle BFC=\angle BGC=\angle BHC=90^o`. As discussed in the preceding example, :math:`BCE` is also a right angle. Thus, a tangent is perpendicular to the segment joining the center of the circle (A) to the point of tangency (C).
...
.. image:: geom_figs/circle/chords.png
  :width: 500
...
.. image:: geom_figs/circle/inner_intersection_angle.png
  :width: 400
...
.. math::
   ...
   \angle BDE &= \frac{1}{2} \angle BAE~\text{(Inscribed angle theorem)}

   \angle DBC &= \frac{1}{2} \angle DAC~\text{(Inscribed angle theorem)}

   \angle BFE &= \angle BDE + \angle DBC~\text{(Exterior angle = sum of remote interior angles)}

   \angle BFE &= \angle CFD~\text{(Vertically opposite angles)}

   &=\frac{1}{2}\left(\angle BAE + \angle DAC\right)~\text{(Putting all the preceding steps together)}
   
	  
======================================
	  
.. image:: geom_figs/circle/outer_intersection_angle.png
  :width: 600
...
.. math::
   ...
   \angle BCE &= \frac{1}{2} \angle BAE~\text{(Inscribed angle theorem)}

   \angle FBC &= \frac{1}{2} \angle DAC~\text{(Inscribed angle theorem)}

   \angle BFE &= \angle BCE - \angle FBC~\text{(Exterior angle = sum of remote interior angles)}

   &=\frac{1}{2}\left(\angle BAE - \angle DAC\right)~\text{(Putting all the preceding steps together)}
  	  
======================================

.. image:: geom_figs/circle/chord_intersection.png
  :width: 500

If BC and DE are two chords that intersect inside the circle at point F, then :math:`BF \times FC = EF\times FD`. This is the **chord-intersection theorem**.
	  
This follows from the similarity of triangles FBE and FDC.

.. math::

   \angle BEF &= \angle DCF~\text{(Inscribed angles subtending the same arc)}

   \angle BFE &= \angle DFC~\text{(Vertically opposite angles)}

   \Delta BFE\sim \Delta DFC~\text{(Similarity by AA test)}


Tangents and Secants
---------------------

.. image:: geom_figs/circle/two_tangents.png
  :width: 600
...
.. image:: geom_figs/circle/tangent_secant.png
  :width: 600
...
.. math::
   ...
   & DB^2 = DK\times DL = DH\times DG

This is the **tangent-secant theorem**.

The proof (which we skip here) is based on :math:`\Delta DKH\sim \Delta DLG` and :math:`\Delta DBK\sim \Delta DBL`.

Incircle
===========
	  
.. image:: geom_figs/basics/incircle.png
  :width: 500
...
.. math::
   ...
   & AB = AC = AD = \text{radius of the circle (the inradius)}

   & \angle AGB = \angle AGC,~\angle AFC=\angle AFD,~\angle AED=\angle AEB
...
Let :math:`r` be the inradius. We also have

.. math::

   \text{Area of }\Delta EFG &= \text{Area of }\Delta AEF + \text{Area of }\Delta AFG + \text{Area of }\Delta AGE

   &= \frac{1}{2}AD\cdot EF+\frac{1}{2}AC\cdot FG+\frac{1}{2}AB\cdot GE

   &= \frac{1}{2}r\cdot\left(EF+FG+GE\right)

   &= \frac{1}{2}r\cdot \text{Perimeter of }\Delta EFG

   \implies \text{Inradius}~&=~\frac{\text{Area}}{\text{Semi-perimeter}}
...
.. image:: geom_figs/basics/circumcircle.png
  :width: 500	  
