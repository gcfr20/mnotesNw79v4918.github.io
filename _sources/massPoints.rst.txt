...
.. image:: geom_figs/mass_pts/center_of_mass.png
  :width: 400
...
.. image:: geom_figs/mass_pts/mass_pts_ex1.png
  :width: 400

Given :math:`BD:DA=3:1` and :math:`BE:EC=1:1`, find :math:`CG:GA`, :math:`AF:FE`, :math:`BF:FG`, and :math:`CF:FD`.
...
.. image:: geom_figs/mass_pts/mass_pts_ex1_res.png
  :width: 400

.. math::

   & \frac{CG}{GA}=\frac{\text{Mass at A}}{\text{Mass at C}}=\frac{9}{3}=3

   & \frac{AF}{FE}=\frac{\text{Mass at E}}{\text{Mass at A}}=\frac{6}{9}=\frac{2}{3}

   & \frac{BF}{FG}=\frac{\text{Mass at G}}{\text{Mass at B}}=\frac{12}{3}=4

   & \frac{CF}{FD}=\frac{\text{Mass at D}}{\text{Mass at C}}=\frac{12}{3}=4
...
.. image:: geom_figs/mass_pts/centroid.png
  :width: 400
...
.. math::
   ...
   & \frac{AD}{DB}=\frac{BE}{EC}=\frac{CG}{GA}=1
   
   & \frac{AF}{FE}=\frac{BF}{FG}=\frac{CF}{FD}=2
	  
=================================================

.. image:: geom_figs/mass_pts/cevas.png
  :width: 400
...
.. math::
   ...
   \frac{AD}{DB}\cdot \frac{BE}{EC}\cdot \frac{CG}{GA} & = \frac{v}{u}\cdot \frac{w}{v}\cdot \frac{u}{w}=1
...
.. image:: geom_figs/mass_pts/quads_mp.png
  :width: 400
...
Here is another example where mass points approach needs to be supplemented by other geometrical properties.

.. image:: geom_figs/mass_pts/area_sim_1.png
  :width: 400

Given in rectangle ABCD, BD=4, AB=2, AE=EC, and CF=FD, find the area of the shaded triangle.

Triangle BCD is a good candidate for the application of mass points as two Cevians are visible. However, we have only one ratio to work with CF/FD=1. We need two ratios to resolve the traingle using mass points.

We get the second ratio using similarity. Triangles EGC and DGB are similar. Moreover, BD/EC=2. Hence, BG/GC=2. There is our second ratio.

So we assign mass 1 to B, 2 to C (to account for BG/GC=2). Then, we assign mass 2 to point D (to account for CF/FD=1). The masses for G, F, and H can then be deduced. See figure below.

.. image:: geom_figs/mass_pts/area_sim_1_mp.png
  :width: 400

The area ratios can then be deduced as follows:

.. math::

  &\text{Area of }\Delta BCD=\frac{1}{2}~\text{Area of rectangle ABCD}=4

  &\frac{\text{Area of }\Delta BDG}{\text{Area of }\Delta CDG}=\frac{BG}{GC}=2~\text{(Triangles with same height have areas in the ratio of the bases)}

  &\implies \text{Area of }\Delta BDG=\frac{2}{3}\times\text{Area of }\Delta BCD=\frac{8}{3}

    &\frac{\text{Area of }\Delta BHG}{\text{Area of }\Delta BHD}=\frac{GH}{HD}=\frac{2}{3}~\text{(Triangles with same height have areas in the ratio of the bases)}

  &\implies \text{Area of }\Delta BHG=\frac{2}{5}\times\text{Area of }\Delta BDG=\frac{2}{5}\times\frac{8}{3}=\frac{16}{15}
