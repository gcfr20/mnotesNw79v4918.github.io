...
The probability of having the first 3 tosses heads and the rest tails is :math:`\left(\frac{1}{2}\right)^7`. This is one ordering of heads and tails. How many orderings are possible? We need to select 3 out of 7 positions for heads, the rest will be tails. This can be done in :math:`C(7,3)` ways. Thus, the probability of getting exactly 3 heads and 4 tails in any order is

.. math::

   C(7,3)\times \left(\frac{1}{2}\right)^7
...
The set :math:`u` contains all the animals in a jungle, the set :math:`A` contains all the carnivorous animals in the jungle, the set :math:`B` contains all the mammals in the jungle, and set :math:`C` contains all the animals in the jungle that can climb trees.
...
The notation :math:`P(A|B)` is for probability of A given B is true, or it is the probability of A conditional on B.

We can simplify this as

.. math::

   P(A|B) = \frac{\#A\cap B}{\#B} = \frac{\frac{\#A\cap B}{\#u}}{\frac{\#B}{\#u}}=\frac{P(A\cap B)}{P(B)}.

Thus,

.. math::

   P(A|B) = \frac{P(A\cap B)}{P(B)}~\text{or}~P(A\cap B)=P(A|B)\cdot P(B).

======================================

Consider another situation. The probability of a fair coin resulting in heads when tossed is :math:`\frac{1}{2}`. If we toss the coin and the result is heads, what is the probability that it will result in heads when it is tossed a second time. Clearly, the result of the first toss does not change the probability of getting heads in the second toss. In this case,

.. math::

   P(\text{Heads on the second toss}|\text{Heads on the first toss})=P(\text{Heads on the second toss})

Thus,

.. math::

   & P(\text{Heads on the second toss}~\cap~\text{Heads on the first toss})

   &= P(\text{Heads on the second toss}|\text{Heads on the first toss})\cdot P(\text{Heads on the first toss})

   &= P(\text{Heads on the second toss})\cdot P(\text{Heads on the first toss})
...
.. math::
   ...
   & P(\text{person will get up before 7 today})

   & = P(\text{person will get up before 7}~\cap~\text{today is a weekday}) + P(\text{person will get up before 7}~\cap~\text{today is a weekend})

   & = P(\text{person will get up before 7}|\text{today is a weekday})\cdot P(\text{today is a weekday})

   &+ P(\text{person will get up before 7}|\text{today is a weekend})\cdot P(\text{today is a weekend})

   &= 0.9\times \frac{5}{7} + 0.2\times \frac{2}{7} = 0.7
...
.. math::
   ...
   E[\text{days}],~\left<\text{days}\right>,~\overline{\text{days}}

For our problem:
 
.. math::

   E[\text{days}] & = 1 \times P(1) + 2 \times P(2) + 3 \times P(3) + 4 \times P(4) + 5 \times P(5)

   & = 0.2 + 2\times 0.4 + 3 \times 0.2 + 4\times 0.1 + 5\times 0.1 = 2.5
...
.. image:: probability_figs/markov_chain.png
  :width: 400
...
.. math::
   ...
   & = 0.8\times 0.1 + 0.6\times 0.4 + 0.5\times 0.5 = 0.57
...
Note that 0.28 + 0.15 + 0.57 = 1 (these are again mutually exclusive exhaustive events). A, B, and C are the only possibilities where Mary could be on Tuesday and these are disjoint possibilities. She cannot be in more than one city at the
same time.

Let us simplify our notation along the following lines: :math:`P(\text{X end}|\text{Y start})` as :math:`P_{X\rightarrow Y}`, and :math:`P(\text{X on Tue})` as :math:`P^X_{\text{Tue}}`.
...
.. math::
   ...
   \end{bmatrix} =
   \begin{bmatrix}
   P_{A\rightarrow A} &    P_{B\rightarrow A}  &    P_{C\rightarrow A} \\
   P_{A\rightarrow B} &    P_{B\rightarrow B}  &    P_{C\rightarrow B} \\
   P_{A\rightarrow C} &    P_{B\rightarrow C}  &    P_{C\rightarrow C} \\
   \end{bmatrix}
   \begin{bmatrix}
   P^A_{\text{Mon}}  \\
   P^B_{\text{Mon}} \\
...
.. image:: probability_figs/mc_ex_1.png
  :width: 700
...
.. math::
   ...
   \textbf{Initial probabilities:}~& P^1_0 = 1,~P^2_0 = 0,~P^3=0,~P^4_0 = 0

   \textbf{After hop 1:}~& P^1_1 = 0,~P^2_1 = x,~P^3_1=x,~P^4_1 = x

   \textbf{After hop 2:}~& P^1_2 = 3x^2,~P^2_2 = 2x^2,~P^3_2=2x^2,~P^4_2 = 2x^2

   \textbf{After hop 3:}~& P^1_3 = 6x^3,~P^2_3 = 7x^3,~P^3_3=7x^3,~P^4_3 = 7x^3

   \textbf{After hop 4:}~& P^1_4 = 21x^4,~P^2_4 = 20x^4,~P^3_4=20x^4,~P^4_4 = 20x^4

   &\implies P^1_4 = 21x^4 = \frac{21}{3^4}=\frac{7}{3^3}=\frac{7}{27}
...
.. math::
   ...
   P^1_4 &= P_{1\rightarrow 1}\cdot P^1_3 + P_{2\rightarrow 1}\cdot P^2_3 + P_{3\rightarrow 1}\cdot P^3_3 + P_{4\rightarrow 1}\cdot P^4_3

   &= 3\times P_{2\rightarrow 1}\cdot P^2_3 = 3\times \left(7x^3\right)\cdot x=21x^4

   P^2_4 &= P_{1\rightarrow 2}\cdot P^1_3 + P_{2\rightarrow 2}\cdot P^2_3 + P_{3\rightarrow 2}\cdot P^3_3 + P_{4\rightarrow 2}\cdot P^4_3

   &= P_{1\rightarrow 2}\cdot P^1_3 + 2\times P_{2\rightarrow 1}\cdot P^2_3 = \left(6x^3\right)\cdot x +  2\times \left(7x^3\right)\cdot x = 20x^4
...
By symmetry, the probability that the cricket is on any of the leaves 2, 3, or 4 after :math:`n-1` hops is

:math:`\frac{1}{3}\left(1-p_{n-1}\right)`,

where :math:`p_{n-1}` is the probability that the cricket is on leaf 1 after n-1 hops.

The probability of hopping to leaf 1 from any one of these leaves in the next hop is 1/3.

Therefore, the probability of getting to leaf 1 in the nth step is

:math:`p_n=3\times \frac{1}{3}\times \frac{1}{3}\left(1-p_{n-1}\right)=\frac{1}{3}\left(1-p_{n-1}\right)`.
...
.. math::
   ...
   p_2 &= \frac{1}{3} \left(1-p_1\right) = \frac{1}{3}

   p_3 &= \frac{1}{3} \left(1-p_2\right) = \frac{2}{9}

   p_4 &= \frac{1}{3} \left(1-p_3\right) = \frac{7}{27}
...
.. image:: probability_figs/prob_tree_bball.png
  :width: 400
...
.. math::
   ...
   P(\text{Leo wins}) = P(\text{Leo wins on throw 2}) + P(\text{Leo wins on throw 4}) + P(\text{Leo wins on throw 6}) + \ldots
...
.. math::
   ...
   P(\text{Leo wins}) & = 0.7 \times 0.5 + 0.7 \times 0.5 \times 0.7 \times 0.5 + 0.7\times 0.5\times 0.7 \times 0.5 \times 0.7 \times 0.5 + \ldots

   & = 0.35 + 0.35^2 + 0.35^3 + \ldots

   & = \frac{0.35}{1-0.35} = \frac{35}{65}=\frac{7}{13}

We have used the formula for adding an infinite geometric progreassion from our :ref:`Adding geometric series discussion  <algebraicidentities:Adding geometric series>`.

Similarly, we can compute the probability that Srini wins.

.. math::

   P(\text{Srini wins}) & = P(\text{Srini wins on throw 1}) + P(\text{Srini wins on throw 3}) + P(\text{Srini wins on throw 5}) + \ldots

   & = 0.3 + 0.7 \times 0.5 \times 0.3 + 0.7\times 0.5\times 0.7 \times 0.5 \times 0.3 + \ldots

   & = 0.3\times \left(1 + 0.35 + 0.35^2 + \ldots \right)

   & = \frac{0.3}{1-0.35} = \frac{30}{65}=\frac{6}{13}

Surprise, surprise ... :math:`P(\text{Leo wins}) + P(\text{Srini wins}) = \frac{7}{13}+\frac{6}{13}=1`.
